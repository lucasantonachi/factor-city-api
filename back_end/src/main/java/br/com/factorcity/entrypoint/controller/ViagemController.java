...
public class ViagemController {
    ...
    public ResponseEntity<Page<VeiculoResponse>> getAll(@PageableDefault(sort = "idViagem", direction = Sort.Direction.ASC, page = 0, size = 10) Pageable paginacao) {
        Page<ViagemTable> list = this.viagemUseCase.getAll(paginacao);
        return new ResponseEntity(ViagemMapper.tablePageToResponsePage(list), HttpStatus.OK);
    }
    ...
    public ResponseEntity<ViagemResponse> getById(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ViagemMapper.tableToResponse(viagemTable));
    }
    ...
    public ResponseEntity<List<ViagemResponse>> getAllByUsuario(@PathVariable Long idVeiculo) {
        ...
        List<ViagemTable> viagemResponses = this.viagemUseCase.getAllByVeiculo(idVeiculo);
        return new ResponseEntity(ViagemMapper.tableListToResponseList(viagemResponses), HttpStatus.OK);
    }
    ...
    public ResponseEntity<ViagemResponse> createVeiculo(@RequestBody @Valid ViagemRequest viagemRequest, UriComponentsBuilder uriComponentsBuilder) {
        ...
        return ResponseEntity.created(uri).body(ViagemMapper.tableToResponse(viagemTable));
    }
}
